...
public:
  ...
  Eigen::VectorXd kinematicsModel(Eigen::VectorXd x, Eigen::VectorXd u)
  {
    Eigen::VectorXd diff_pose(x.rows());

    diff_pose(STATE::YAW) = u(1) * dt_;
    if (std::fabs(u(1)) < 1e-10) {
      diff_pose(STATE::X) = u(0) * std::cos(x(STATE::YAW)) * dt_;
      diff_pose(STATE::Y) = u(0) * std::sin(x(STATE::YAW)) * dt_;
    } else {
      diff_pose(STATE::X) =
        (u(0) / u(1)) * (std::sin(x(STATE::YAW) + u(1) * dt_) - std::sin(x(STATE::YAW)));
      diff_pose(STATE::Y) =
        (u(0) / u(1)) * (-std::cos(x(STATE::YAW) + u(1) * dt_) + std::cos(x(STATE::YAW)));
    }
  ...
  void predict(const Eigen::VectorXd u)
  {
    ...
    const double dt_yaw = u(1) * dt_;

    Eigen::MatrixXd M = Eigen::MatrixXd::Identity(2, 2);
    M(0, 0) = model_noise_(0) * std::fabs(u(0)) / dt_ + model_noise_(1) * std::fabs(u(1)) / dt_;
    M(1, 1) = model_noise_(2) * std::fabs(u(0)) / dt_ + model_noise_(3) * std::fabs(u(1)) / dt_;
